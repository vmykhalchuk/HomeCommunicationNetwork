...
GSMInitializeProcessor::GSMInitializeProcessor(byte gsmModuleResetPin, bool enableClip, GSMUtils* gsmUtils)
{
...
GSMInitializeProcessor::State GSMInitializeProcessor::processState()
{
  ...
  else if (_state == State::ENABLING_CLIP)
  {
    gsmUtils->getGsmComm()->println("AT+CLIP=1"); gsmUtils->getGsmComm()->flush();
    _setTimer(GSM_NO_RESPONSE_TIMEOUT_SECONDS);
    _state = State::WAITING_FOR_CLIP_ENABLED;
  }
  //else if (_state == State::WAITING_FOR_CLIP_ENABLED) {} // wait for "OK"
...
bool GSMInitializeProcessor::gsmLineReceivedHandler(byte gsmLineReceived, char* lineStr, int lineSize)
{
  ...
  else if (gsmLineReceived == GSM_LINE_OK)
  {
    ...
    else if (_state == State::WAITING_FOR_CLIP_ENABLED)
    {
      _state = State::SUCCESS;
      _debugln(F("GSM MODULE INITIALIZED & CLIP activated!"));
      _deactivateTimer();
      return true;
    }
  }
